...
/**
 * Sanitizes request body to handle cases where objects that should be strings
 * are passed (e.g., DOM elements, proxy objects). Attempts to extract string
 * values from suspicious objects.
 */
function sanitizeRequestBody(body: unknown): unknown {
  ...
  if (typeof body === "object") {
    ...
    // Check if this looks like a DOM element, proxy object, or Trusted Types object
    // Common indicators: isTrusted, _vts, or other non-standard properties
    if (("isTrusted" in obj || "_vts" in obj) && typeof obj !== "string") {
      // Try to extract a value property if it exists
      if ("value" in obj && typeof obj.value === "string") {
        console.warn(
          `[Requesting] Sanitized suspicious object (isTrusted/_vts) to string value: ${obj.value}`
        );
        return obj.value;
      }
      // Try to find a string property that might be the actual value
      // Look for properties that don't start with underscore and aren't boolean/number
      for (const [key, value] of Object.entries(obj)) {
        if (
          ...
          console.warn(
            `[Requesting] Sanitized suspicious object (isTrusted/_vts), extracted '${key}': ${value}`
          );
          return value;
        }
      }
      // Try to find any string that looks like an ID (UUID format or long string)
      for (const [key, value] of Object.entries(obj)) {
        if (typeof value === "string" && value.length > 10) {
          console.warn(
            `[Requesting] Sanitized suspicious object (isTrusted/_vts), using '${key}' as value: ${value.substring(0, 50)}...`
          );
          return value;
        }
      }
      // Last resort: log error and return string representation (this will likely fail downstream)
      console.error(
        `[Requesting] ERROR: Received suspicious object that should be a string but could not extract value: ${JSON.stringify(obj)}. ` +
        `This likely indicates a client-side bug where a DOM element or Trusted Types object was sent instead of a string value.`
      );
      return String(obj);
    }
...
export function startRequestingServer(
    ...
    for (const method of methods) {
      ...
      app.post(route, async (c) => {
        try {
          ...
          const sanitizedBody = sanitizeRequestBody(body) as Record<string, unknown>;
          const result = await concept[method](sanitizedBody);
  ...
  app.post(routePath, async (c) => {
    try {
      ...
      // Sanitize the request body to handle DOM elements, proxy objects, etc.
      const sanitizedBody = sanitizeRequestBody(body) as Record<string, unknown>;
